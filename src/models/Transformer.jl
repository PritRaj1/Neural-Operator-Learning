...
function PositionalEncoding()
    ...
    pe_vector = Float32.(reshape(pe_vector, d_model, max_len, 1))
...
model = createTransformer(251) #|> gpu
x = rand(Float32, 251, 5) #|> gpu
y = rand(Float32, 251, 5) #|> gpu

println(size(model(x, y)))
println(size(y))