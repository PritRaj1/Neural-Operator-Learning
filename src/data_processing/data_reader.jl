...
function load_darcy_data()
    ...
    a_train = parse(Float32, train_matfile["a_field"])
    u_train = parse(Float32, train_matfile["u_field"])
    a_test = parse(Float32, test_matfile["a_field"])
    u_test = parse(Float32, test_matfile["u_field"])
